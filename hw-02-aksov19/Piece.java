...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		body = deepCopyPoints(points);

		// Sort points
		Arrays.sort(body, new Comparator<>() {
			@Override
		...
		Arrays.fill(skirt, points.length);
		for (TPoint point : points) {
	...
	*/
	...
	public Piece computeNextRotation() {
		// Copy body
		TPoint[] newPoints = deepCopyPoints(body);

		// Flip x and y and mirror
		for(int i = 0; i < body.length; i++){
			int x = body[i].x;
			int y = body[i].y;
			newPoints[i].x = height - y - 1;
			newPoints[i].y = x;
		}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while( !rotation.equals(root) ){
			cur.next = rotation;
			cur = rotation;
			rotation = cur.computeNextRotation();
		}
		cur.next = root;
	...
	*/
	...
	private TPoint[] deepCopyPoints(TPoint[] from){
		TPoint[] to = new TPoint[from.length];

		for(int i = 0; i < from.length; i++){
			to[i] = new TPoint(from[i]);
		}
